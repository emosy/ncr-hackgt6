...
def combine_data_by_time(data, time_index, delta=timedelta(hours=1), indices={0,1}):
    ...
    while i < len(data[time_index]):
        ...
        while i < len(data[time_index]) and parser.isoparse(data[time_index][i]) < current_time_block_start + delta:
            new_data[len(indices)].append(datetime.datetime.isoformat(current_time_block_start))
            for ind in indices:
...
all_data = get_all_data(time="2019-10-23T12:00:00Z")
...
print(sorted_data_thing[0])
# zip_data_thing = zip(sorted_data_thing)
data_by_time = combine_data_by_time(sorted_data_thing,4,delta=timedelta(minutes=5))
print(data_by_time[2])
max = max(data_by_time[0])
print()
print(max)
print()
print(data_by_time[2][-1])
